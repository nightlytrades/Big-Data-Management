    ...
    try {
      ...
      command match {
        case "count-all" =>
          ...
          val query = spark sql "SELECT count(*) FROM log_lines" first // TODO 9: write SQL query to count the total number of rows in `log_lines`
          val count: Long = query.getAs[Long](0)
        ...
        case "code-filter" =>
          ...
          val query : String = "SELECT count(*) FROM log_lines WHERE response=200;" //TODO 10: write SQL query to filter view `log_lines` by `responseCode`
        ...
        case "time-filter" =>
          ...
          val query: String = "SELECT count(*) FROM log_lines WHERE time BETWEEN 807274014 AND 807283738;" // TODO 11: write SQL query to filter view `log_lines` by `time` between `from` and `to`
          val count: Long = spark.sql(query).first().getAs[Long](0)
        ...
        case "count-by-code" =>
          ...
          println("Code,Count")
          val query: String = "SELECT response, SUM(bytes) FROM log_lines GROUP BY response ORDER BY response;" // TODO 12: write SQL query the get the count for each `response`, results must be ordered by `response` in ascending order
        ...
        case "avg-bytes-by-code" =>
          ...
          val query: String = "SELECT response, SUM(bytes) FROM log_lines GROUP BY response ORDER BY response;" // TODO 14: write SQL query the get the average of `bytes` for each `response`, results must be ordered by `response` in ascending order
            spark.sql(query).foreach(row => println(s"${row.get(0)},${row.get(1)}"))
        case "top-host" =>
